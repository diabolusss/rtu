...
implementation uses crt,ld5inter,ld5graph;
 ...
 procedure info;
  BEGIN
   ...
   delay(maxRec);
   setContour(minX+1,minY+4,maxX-4,maxY-6,defaultTxtClr);textColor(defaultClr);
   write('Darba uzdevums',br);
   delay(maxRec);
   write('  Izveidot datorizetu datu bazi autoveikalam,',br);   delay(maxRec);
   write('kura lietotajs varetu glabat sekojosu informaciju:',br);   delay(maxRec);
   write('    - masinas marku,',br);   delay(maxRec);
   ...
   write('  Paredzet funkcijas un proceduras sekojosam darbam:',br);   delay(maxRec);
   write('    * apskatit faila saturu,',br);   delay(maxRec);
   write('    * ierakstit faila jaunu informaciju,',br);   delay(maxRec);
   write('    * izslegt no faila informaciju,',br);   delay(maxRec);
   write('    * izslegt no faila informaciju,',br);   delay(maxRec);
   write('    * koriget informaciju,',br);   delay(maxRec);
   write('    * sakartot informaciju faila pec masinas markam un izlaiduma gadiem,',br);   delay(maxRec);
   write('    * atrast visas automasinas, kuru cenas neparsniedz uzdotu,',br);   delay(maxRec);
   write('    * aprekinat automasinu videjo cenu.');   delay(maxRec);
   readkey;
  END;
 ...
 procedure menu;
   ...
   REPEAT
    if ok then begin
     header;footer(5);
     setContour(minX+14,minY+4,length(menuPoints[1]),minY+high(menuPoints)+1,menuClr);
     for i:=1 to high(menuPoints) do begin
   ...
   case arrow of
    ...
    #80:if choosen<high(menuPoints) then begin
         inc(choosen);
         textBackground(defaultTxtClr);textColor(defaultClr);
         gotoXY(minX,choosen);write(menuPoints[choosen]);
         textBackground(defaultClr);textColor(defaultTxtClr);
   ...
   function NOD:word;
    ...
    BEGIN
     ...
     repeat gotoXY(minX,minY);clreol;
      ...
      {$I-}read(tNOD);{$I+}
     until (ioresult=0)and(tNOD>=0)and(tNOD<=maxRec);
     nod:=tNod;
     glBug:=111;
    END;
     ...
     for i:=n*lineCount to y do begin
       ...
       if not eof(foundFileLink) then begin
        ...
        with carRec do begin
         write('#',i+1,nbsp:(5-numCount(id)),name,nbsp:(maxLength-length(name)+1),year,
