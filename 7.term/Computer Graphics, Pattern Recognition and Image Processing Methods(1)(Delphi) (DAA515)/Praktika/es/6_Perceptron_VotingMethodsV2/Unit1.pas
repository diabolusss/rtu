...
begin
  ...
  begin
    ...
    begin
      ...
      if((rg1.ItemIndex=1) or (rg1.ItemIndex=2)) and (itterations mod strToInt(edt15.Text)=0) then
      begin
        reserr[i].er:=error;
        error:=0;
      end;
...
begin
  ...
  begin
    ...
    Cells[2,0]:='Results';
      ...
      begin
        ...
        Cells[2,i]:=IntToStr(reserr[i-1].rs);
      end;
    end;

    repeat
      f:=False;
      for i:=0 to perc_count-2 do
      begin
  ...
  begin
    ...
    lbl8.Caption:='Using result of ' + reserr[0].pr + '(' + IntToStr(reserr[0].rs) + '),' + reserr[1].pr + '(' + IntToStr(reserr[1].rs) +'),' + reserr[2].pr + '(' + IntToStr(reserr[2].rs) +'),' + reserr[3].pr + '(' + IntToStr(reserr[3].rs) + ') and ' + reserr[4].pr + '(' + IntToStr(reserr[4].rs) +')';
  ...
  begin
    ...
    begin
      z:=(a[i]/err_all);
      kju:=(reserr[i].rs)*z;
      res_sum2:=res_sum2+kju;
    end;

    edt14.Text:=IntToStr(err_all);

    if(res_sum2>=(0.5)) then
    begin
      res:=1;
      lbl9.Caption:=FloatToStr(res_sum2) + ' > ' + '0.5';
    end
    else
    begin
      res:=0;
