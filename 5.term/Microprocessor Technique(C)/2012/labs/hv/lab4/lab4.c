...
ISR(ADC_vect){
	...
	Otherwise, ADCL must be read first, then ADCH.(studentu cakarewanai xD) 
	use ADC instead*/

	//LA = ADCL;

	ten_bit_value = ADC;
	//ten_bit_value = ADLAR == 1 ? (LA >> 6) | (ADCH << 2) : LA | (ADCH << 8);
...
void init_devices(void){
	//ADC SETUP
	ADCSRA |= (1 << ADPS2) | (1 << ADPS1) | (1 << ADPS0); // divide by 64 /Set ADC prescalar to 16 - 125KHz sample rate @ 16MHz 
